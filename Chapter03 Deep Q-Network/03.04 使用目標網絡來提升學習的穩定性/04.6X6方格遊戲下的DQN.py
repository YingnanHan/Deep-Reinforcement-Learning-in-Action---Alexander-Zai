...
model = torch.nn.Sequential(
    ...
    torch.nn.ReLU(),
    torch.nn.Linear(L3,L3*2),
    torch.nn.ReLU(),
    torch.nn.Linear(L3*2,L3),
    torch.nn.ReLU(),
...
for i in range(epochs):
    ...
    state1_ = game.board.render_np().reshape(1,6*6*4) + np.random.rand(1,6*6*4)/100.0
    ...
    while(status == 1):
        ...
        state2_ = game.board.render_np().reshape(1,6*6*4) + np.random.rand(1,6*6*4)/100.0
...
def test_model(model,mode="static",display=True):
    i = 0
    game = Gridworld(size=6,mode = mode) # 產生一場測試遊戲
    state_ = game.board.render_np().reshape(1,6*6*4)+np.random.rand(1,6*6*4)/10.0
